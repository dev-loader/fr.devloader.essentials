...
namespace Devloader.Shaders
{
    ...
    public class BuoyantObject : MonoBehaviour
    {
        ...
        [Header("Water")]
        [SerializeField] private Transform waterReference;
        ...
        {
            if(waterReference)
                waterHeight = waterReference.position.y;

            if(materialReference)
            {
                steepness = materialReference.GetFloat("_WaveSteepness");
                wavelength = materialReference.GetFloat("_WaveLength");
                speed = materialReference.GetFloat("_WaveSpeed");

                Vector4 direction = materialReference.GetVector("_WaveDirection");
